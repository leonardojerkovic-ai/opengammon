...
- **Faza 2 (svjesno odstupanje od "faza po faza", dogovoreno s korisnikom 2026-09-23 — Faza 1 nije zatvorena):**
  ...
  - **Jednostrana bearoff baza gotova, sa svim brojkama na jednom mjestu:** DP (dva pravila odabira poteza, `finish`+`first_off`), GNUbg usporedba na svih 54 264 pozicije (8 poznatih, dokumentiranih iznimaka — near-tie, prihvaćeno), kvantizacija `f64→u16` (najveća greška ~6×10⁻⁵), format na disku + mmap (samoopisujuće zaglavlje, little-endian, iscrpno round-trippano), brzina dohvata **891 ns prosjek** (raspon 809-929 ns kroz tri mjerenja, min 200-300 ns; najgori slučaj OS-ovisan, ne dizajn). Poznat dug: `finish`/`first_off` dohvat alocira `Vec<f64>` po pozivu — u redu sada, treba riješiti prije Faze 3 (rollout unutarnja petlja) — vidi `docs/backlog.md`.
  - **Faza 2 ostaje otvorena: preostaje dvostrana baza.** Prije bilo čega novog: natrag na Fazu 1 (milijunski run, ~11% gotovo) — Faza 2 ostaje svjesno odstupanje dok se Faza 1 formalno ne zatvori.
